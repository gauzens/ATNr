...
**Note: update that part once we found some proper names for the models**
...
* The object describing the interactions between plants and nutrients (`$K` or `$V`) are matrices for which the number of rows equals to the number of nutrients and a number of columns matches the number of basal species (this point is specific to the Schneider model which is the only one including an explicit dynamic of the nutrient pool). 


To run the population dynamic, all the parameters must be defined. It is possible to automatically load a by default parametrisation using the dedicated functions:
...
## Running the population dynamic
...
The function checks for the presence of several connected components (i.e. if the food web is composed of disconnected sub networks). If that happens, the food web still returned, but a warning is raised. To handle this warning in order to obtain a food web that has not disconnected components, a `tryCatch` can be implemented:

```{r, fig.width=4, fig.height=4, fig.align='center'}
fw <- NULL
while (is.null(fw)) { #keep creating a food web until there is no warning
  fw <- tryCatch(
    create_niche_model(S, C),
    warning = function(w) NULL
  )
}
show_fw(fw, title = "Niche model food web")
```
...
It can happen that some body mass structures lead to food webs with disconnected species (consumers without preys, or isolated basal resources). The absence of such species can be guaranteed using a `tryCatch`:

```{r}
L <- NULL
i <- 0 #counter to exit loop after 100 iterations
while(is.null(L)) {
  if (i >= 100) break #this exits the loop
  L <- tryCatch(
...
_ATNr_ makes it relatively easy to vary one parameter to assess its effect on the population dynamic. For example, we can study how changes in temperatures from 15 to 30 Celsius degrees affects the number species to go extinct. 
...
## Effect of species richness on extinction probabilities

```{r}
# defines initial parameters
C = 0.25 # connectance
richness <-  seq(20, 60, by = 5)
times <- seq(0, 15000, 150)
persistence <- rep(NA, length(richness))
# run a loop over connectance values
i <- 0
for (S in richness){
  i <- i + 1
  # 2) create the food web using the niche model
  b <- TRUE
  while(b){
    # here we want to make sure that the niche model function returns a valid
    # food web for instance, it is possible to obtain a food web that is
    # composed of disconnected subparts in that case the create_niche_model
    # function raise a warning here we want to make sure to not have this case.
    # If it happens, we affect NA to fw and at the end of the while loop check
    # if fw is equal to NA. If yes, we restart the process
    fw <- tryCatch({
      a.fw <- create_niche_model(S, C)
      basals <- which(colSums(a.fw) == 0)
      non.basals <- which(colSums(a.fw) != 0)
      # order the food web to have basal species first
      a.fw <- a.fw[c(basals, non.basals), c(basals, non.basals)]
    },
    error = function(e) NULL,
    warning = function(w) NULL
    )
    # at that point, fw is either NULL or a valid food web
    
    # set body masses as a function of trophic level
    TL <- tryCatch(
      TroLev(fw),
      error = function(e) return(NULL)
    )
    # here TL is either a vector of trophic levels ofr NULL
    # if fw or TL were not correctly calculated, restart the food web generation
    b <- is.null(fw) || is.null(TL)
  }
  # define body masses based on trophic levels
  masses <- 0.001 * 100 ^ (TL - 1 + rnorm(length(TL), 0, 0.01))
  
  # create the model object. 
  n_basal = sum(colSums(fw) == 0)
  model <- create_model_Scaled(S, n_basal, masses, fw)
  model <- initialise_default_Scaled(model)
  model$ext <- 1e-4 #NOTE: changed this to get some extinctions
  # model$c = 0.1
  model$K <- 10
  model$q <- 1.2
  ...
  persistence[i] <- sum(sol[nrow(sol), -1] < model$ext) / S
}
```

```{r, fig.width=4, fig.height=3, fig.align='center'}
plot(richness, persistence,
     pch = 20, cex = 0.5, ylim = c(0, max(persistence)), frame = FALSE,
     ylab = "Proportion of Extinctions", xlab = "Species Richness")
lines(richness, persistence, col = 'blue')
```

## Effect of predator-prey body mass ratio and temperature on species persistence

Predator-prey body mass ratio and envorinment temperature have been shown to affect persistence of species in local communities, e.g. @binzer2016interactive. Here, we use the _ATNr_ model (**name here**) to replicate the results from @binzer2016interactive. In particular, we compute the fraction of species species that persist for predator-prey body mass ratio values in $\left[ 10^{-1}, 10^4 \right]$ and temperature values in $\{0, 40\}$ °C.
...
while (is.null(fw)) { #keep creating a food web until there is no warning
  ...
  if (is.null(TL)) fw <- NULL
}
biomasses <- runif(S, 2, 3)
for (scal in scaling) {
  masses <- 0.01 * scal ^ (TL - 1)
  range(masses)
  sorted <- sort_input(masses, fw)
  masses <- sorted$body.mass
  fw <- sorted$food.web
...
Similarly to @binzer2016interactive, species persistence increases with increasing values of predator-prey body mass ratios and increasing temperature
...
fw <- NULL
TL <- NULL
while (is.null(fw) | is.null(TL)) {
  fw <- tryCatch(
    create_niche_model(S, C = .15),
    warning = function(w) NULL
  )
  TL <- tryCatch(
    TroLev(fw),
...
```
**I'm wondering whether that one is not too trivial?**


## Updating key parameters without creating a new model object

Changing parameters that are used in the `create_model` functions without creating a new model object is almost always a bad idea. Those parameters are important structural parameters, and changing one of them implies changes in most of the other variables contained in the model object. For instance, the example above,  changing the number of species in the model object will lead to inconsistancies in the different variables: the dimensions of objects storing attack rates, body masses and so on won't match the updated number of species. Some basic checks are made before starting the integration in the `lsoda_wrapper` function, based on the `run_checks` procedure also available in the package.   
...
**Note: Should we let the possibility for users to update these key parameters (i.e. the ones in the create_model() functions)? Should we restrict them that they can only be read and not modified? For instance by not exporting them and creating a get_x() method that would return the value of x? This options is much better in terms of "security" but much less straightfoward to use. **
